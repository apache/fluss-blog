...
Filter pushdown works on Arrow-format log tables and data written after statistics are enabled. Pushdown works at batch granularity by skipping non-matching batches. Flink SQL applies the remaining row-level filter automatically while users of Java, Rust, Python, and C++ clients must apply it themselves.
