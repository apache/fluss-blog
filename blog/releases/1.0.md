...
date: 2026-09-21
tags: [announcement]
description: "Fluss 1.0 strengthens the real-time data foundation for AI with Fluss Gateway, native clients, and deeper streaming and lakehouse integration."
image: ./../assets/1.0/banner-1200x510.webp
---
...
Gateway can also serve as an HTTP ingestion path into Lakehouse. When Lakehouse support is enabled for the target table, Fluss's separate tiering service batches the records written through the Gateway and moves them to the Lakehouse within a few minutes. This combines simple HTTP ingestion with minute-level Lakehouse freshness.

### Toward MCP Access for Agents

Building on the HTTP interface available today, the community plans to add **Model Context Protocol (MCP)** support so that agents can interact with Fluss through a standard tool interface. The goal is to make frequently changing business data, user context, and application state easier to access from agent workflows.
...
Filter pushdown works on Arrow-format log tables and only for data written after statistics are enabled. Flink SQL applies the remaining row-level filter automatically, while users of Java, Rust, Python, and C++ clients must apply it themselves.
...
Bucket rescaling does not currently apply to non-partitioned tables, tables using the aggregation merge engine, or tables with historical partition access enabled. The relevant clients and connectors must be upgraded before enabling it.
...
### Server-Side Scans for Interactive Reads of Primary Key Tables

Fluss 1.0 adds a server-side scan path for primary key tables. TabletServers scan the current data in a primary key table directly and return it to the client in a sequence of small batches. This avoids downloading snapshot files to the client and replaying subsequent changelog records.

The server-side strategy is intended for interactive queries over small primary key tables, such as dashboard refreshes over a few thousand rows or agent tool calls that need fresh context with low latency. It keeps stateful scan sessions on TabletServers, so it is not a general replacement for the default snapshot-merge strategy used for large tables or high-concurrency batch reads. Java applications can use `table.newScan().createBatchScanner()`, and Flink batch queries can opt in with `client.scanner.kv.batch-strategy = 'server-scan'`.
...
An agent answering a question about past activity, or an application enriching a user profile, may need business context that has already moved out of Fluss hot storage. Late events, status updates, and data corrections can also arrive after a partition has been removed. Version 1.0 introduces historical partition access, currently available with the Paimon lake format, so that this data can continue to serve real-time applications.

For partitions removed from Fluss but still retained in Paimon, primary key point lookups continue to run on the Fluss server. When the original hot partition no longer exists, the TabletServer queries the corresponding Paimon table directly. On the first access to the relevant data files, the server builds a local lake lookup cache in Fluss. Subsequent point lookups can reuse this cache, reducing repeated remote reads and speeding up access.
...
-- 2. RoaringBitmap aggregation table: continuously accumulate unique visitors by channel and hour
...
-- 4. Aggregate across dimensions in batch mode: merge bitmaps, then compute exact unique visitor counts
...
The window is a half-open interval, `[start, end)`. Log tables return records appended within the window. For each key that changed during the window, primary key tables return the latest valid value as of the end of the window. Delete events are not returned in the results.
...
Fluss 1.0 adds Coordinator high availability. When multiple Coordinators are deployed in the same cluster, one acts as the leader and the others remain on standby. If the leader fails, a standby is elected to take over metadata management and recovery coordination.
...
Fluss 1.0 supports configuring multiple local data directories for a TabletServer through `data.dirs`, distributing logs and KV data across several disks. When creating a new replica, the TabletServer chooses the least-loaded disk, balancing log and primary key replicas independently. This reduces the risk of a single disk becoming a capacity or I/O bottleneck and makes it easier to expand storage on a node.
...
Fluss 1.0 adds several layers of resource protection to help clusters remain available under disk, memory, or request pressure caused by overload and uneven traffic distribution:
...
- **KV capacity admission control**: The creation of new primary key table or partition replicas is limited so that RocksDB instances do not consume too much TabletServer memory.
...
Fluss 1.0 introduces `Admin.getClusterHealth()`, which checks active leaders and in-sync replicas (ISR) to determine whether a cluster has recovered. Its primary purpose is to support graceful rolling upgrades. After each TabletServer restart, the upgrade process waits for the cluster to return to a Ready state before proceeding to the next node. This avoids restarting additional nodes while leader elections or replica catch-up are still in progress, reducing the risk of multiple unrecovered nodes, insufficient ISR, and temporary data unavailability.
...
For security management, Fluss now supports centralized management of multiple SASL/PLAIN users. Administrators maintain users through the dynamic `security.sasl.plain.credentials` configuration, use `sys.append_cluster_configs` to add users and passwords, and use `sys.subtract_cluster_configs` to remove users, all without restarting the cluster. Clients set `client.security.protocol` to `SASL` and `client.security.sasl.mechanism` to `PLAIN`, then authenticate with `client.security.sasl.username` and `client.security.sasl.password`.

Learn more: [Monitoring Metrics](https://fluss.apache.org/docs/1.0/maintenance/observability/monitor-metrics/), [Authentication](https://fluss.apache.org/docs/1.0/security/authentication/), [Cluster Configuration Procedures](https://fluss.apache.org/docs/1.0/engine-flink/procedures/#cluster-configuration-procedures).
...
Fluss 1.0 would not have been possible without the community. Thank you to the contributors below who participated in development, issue reporting, reviews, testing, documentation, and production validation:

> AlexZhao, Andrea Bozzo, Anton Borisov, Arnav-Panjla, Aryamaan Singh, Beetle brank, CaoZhen, Chase Naples, Daeuk Choi, Dream95, Evan, Femi, ForwardXu, Gabriel Baldez, George Stamatakis, Gezi-lzq, Giannis Polyzos, Giovanny Gutiérrez, GuoYu, Hemanth Savasere, Hongshun Wang, Howie Wang, JB Onofré, Jackeyzhe, Jared Yu (余启正), Jark Wu, Junbo Wang, Junfan Zhang, Kaiqi Dong, Kaixuan Duan, Karan Pradhan, Keith Lee, Kelvin Wu, Leonard Xu, Liebing, Liurnly, Lorenzo Affetti, Madhur Chandran, Mahesh Sambaram, MehulBatra, Miao, Michael Koepf, Mitchell, Muhammet Orazov, NekoPunch, Nicoleta Lazar, Nikhil Negi, Pavlos-Petros Tournaris, Pei Yu, Pengcheng Huang, Prajwal Banakar, Pranav Shukla, QuakeWang, Shawn Huang, SkylerLin, Thorne, Tom, Trevin Chow, Uğur Tafralı, Vaibhav, Vladyslav Banar, XiaoHongbo, Xiaobing Fang, Xuyang, Yang Guo, Yang Wang, Yang Zhang, Yann Byron, aicontentcreate2023-star, bryndenZh, fhan, fluxo, hanliu0830, kaiwangleo, leon.jeon, litiliu, naivedogger, nhuan.bc, nhuantho, seokjin0414, slfan1989, striver619, tison, vamossagar12, vbhanuchander-lang, warmbupt, whenzhou.zc, xiaozhou, xx789, yunhong, yuxia Luo, zhengyunhong.zyh, zhigang, 白鵺.
