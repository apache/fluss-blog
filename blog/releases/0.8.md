...
date: 2025-11-09
tags: [announcement]
description: "Fluss 0.8 expands the Streaming Lakehouse with Iceberg and Lance integration, Delta Joins, and improvements for production deployments."
image: ./../assets/0.8/banner-1200x510.webp
---
...
You can find more detailed instructions in the [Iceberg Lakehouse documentation](/docs/0.8/streaming-lakehouse/integrate-data-lakes/iceberg/).
...
See the [LanceDB blog post](https://lancedb.com/blog/fluss-integration/) for the full integration. You also can find more detailed instructions in the [Lance Lakehouse documentation](/docs/0.8/streaming-lakehouse/integrate-data-lakes/lance/).
...
You can find more detailed instructions in the [Delta Join documentation](/docs/0.8/engine-flink/delta-joins/).
...
You can find more detailed instructions in the [Materialized Table documentation](/docs/0.8/engine-flink/ddl/#materialized-table).
...
- **[Graceful Shutdown](/docs/0.8/maintenance/operations/graceful-shutdown/)**: Fluss supports cluster rolling upgrade, and we introduced a graceful shutdown mechanism for TabletServers in this version. During shutdown, leadership is proactively migrated before termination, ensuring that read/write latency remains unaffected during rolling upgrades.
...
You can find more detailed instructions in the [Updating Configs documentation](/docs/0.8/maintenance/operations/updating-configs/).
...
You can find more detailed instructions in the [Deploying with Helm documentation](/docs/0.8/install-deploy/deploying-with-helm/).
...
However, Fluss 0.8 is the first official release since the project entered the Apache Incubator, and it includes changes such as package path updates (e.g., groupId and Java package names). As a result, applications that depend on the Fluss SDK will need to make corresponding code adjustments when upgrading to version 0.8. Please refer to the [upgrade notes](/docs/0.8/maintenance/operations/upgrade-notes-0.8/) for a comprehensive list of adjustments to make and issues to check during the upgrading process.
